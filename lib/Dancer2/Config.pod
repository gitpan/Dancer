...
document aims to describe how to manipulate Dancer2's configuration settings
(through code or by file), and to document the various settings that are
available in Dancer2.
...
Dancer2 will B< not > guess what type of file format you are using - it will
...
as Dancer2 will not guess the type of format you are using.
...
Dancer2's L<Dancer2::MIME> module uses C<application/data> as a default
...
L<Dancer2::Logger::Syslog>, L<Dancer2::Logger::Log4perl>, L<Dancer2::Logger::PSGI>
...
Dancer2 2 we suggest you use C<log_level> under the Loger engine
configuration:
...
See L<Dancer2::Session> for supported engines and their respective configuration.

=head3 session_expires

The session expiry time in seconds, or as e.g. "2 hours" (see
L<Dancer2::Cookie/expires>.  By default, there is no specific expiry time.
