...
feel free to follow C<@PerlDancer2> for news and updates on Dancer2!).
...
L<http://github.com/sukria/Dancer2/issues>.

=head2 Reporting Bugs

We prefer to have all our bug reports on GitHub, in the issues section:
L<http://github.com/sukria/Dancer2/issues>. It's possible though to report bugs
...
Fork the repository L<http://github.com/sukria/Dancer2> (click "Fork")
...
your clone (C<master> is used only for building releases).
    ...
    $ git remote add upstream https://github.com/sukria/Dancer2.git
    $ git fetch upstream
...
production concerns: Dancer2 now promises to provide an API-stable and
...
L<http://github.com/sukria/Dancer2>.
...
Dancer2 is intended to be a micro-framework. That means among other things that
...
perform a dynamic loading. Dancer2 has a class dedicated to that job:
L<Dancer2::ModuleLoader>. Here is an example of how to use it:

    package Dancer2::Some::Thing;
    use Carp;

    sub init {
        Dancer2::ModuleLoader->load('Some::Deps')
            or croak "the feature provided by Dancer2::Some::Thing needs Some::Deps";
    }

That way, an optional feature doesn't block Dancer2 from being installed since
...
For current information on Dancer2's plans for the future, see the file TODO at
L<https://github.com/sukria/Dancer2/blob/master/TODO>.
